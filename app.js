const state={apiBase:localStorage.getItem("outilsLestoBreizhStopsApi")||"",lines:[],courses:[],selectedCourse:null,run:null,gpsWatch:null,gpsEnabled:false,position:null,boardings:0,alightings:0,onboard:0,events:[],autoArmed:false,validating:false};
...
function updateDistance(){const c=current();if(!c||!state.position){$("currentDistance").textContent=state.gpsEnabled?"Recherche GPS…":"GPS désactivé";return}const d=distanceKm(state.position,{lat:Number(c.lat),lon:Number(c.lon)})*1000;$("currentDistance").textContent=d>=1000?(d/1000).toFixed(1)+" km":Math.round(d)+" m";const scheduled=minutes(c.scheduled_time);if(scheduled!=null){const now=new Date(),nowMin=now.getHours()*60+now.getMinutes()+now.getSeconds()/60;const sec=(nowMin-scheduled)*60;$( "currentDelay").textContent=delayText(sec)}}
async function validateStop(){
  const c=current();if(!c||!state.run?.id||state.validating)return;state.validating=true;const actual=new Date(),scheduled=minutes(c.scheduled_time),nowMin=actual.getHours()*60+actual.getMinutes()+actual.getSeconds()/60,sec=scheduled==null?null:Math.round((nowMin-scheduled)*60),after=Math.max(0,state.onboard+state.boardings-state.alightings);
  try{await api("/api/public/sae/runs/"+encodeURIComponent(state.run.id)+"/validate-stop",{method:"POST",body:JSON.stringify({course_stop_id:c.id,stop_index:state.run.index,actual_time:actual.toISOString(),boardings:state.boardings,alightings:state.alightings,onboard_before:state.onboard,auto:false})});state.events.push({stop:c,actualTime:actual.toISOString(),delaySeconds:sec,boardings:state.boardings,alightings:state.alightings,onboardBefore:state.onboard,onboardAfter:after});state.onboard=after;state.boardings=0;state.alightings=0;state.run.index++;state.autoArmed=false;if(state.run.index>=state.run.course.stops.length){await finishRun();return}renderRun()}catch(e){alert(e.message)}finally{state.validating=false}
}
...
function startGps(){if(!navigator.geolocation){alert("La géolocalisation n’est pas disponible.");return}state.gpsEnabled=true;state.autoArmed=false;$("gpsToggleBtn").textContent="📍 GPS actif";$("gpsToggleBtn").classList.add("primary");state.gpsWatch=navigator.geolocation.watchPosition(p=>{state.position={lat:p.coords.latitude,lon:p.coords.longitude};updateDistance();const cur=current();if(!cur||state.validating)return;const d=distanceKm(state.position,{lat:Number(cur.lat),lon:Number(cur.lon)})*1000;if(d<=45)state.autoArmed=true;const moving=Number.isFinite(Number(p.coords.speed))?Number(p.coords.speed)>1.5:true;if(state.autoArmed&&d>=85&&moving){state.autoArmed=false;validateStop()}},()=>{setStatus("reportStatus","GPS indisponible, la validation manuelle reste disponible.")},{enableHighAccuracy:true,maximumAge:1500,timeout:15000});updateDistance()}
function stopGps(){if(state.gpsWatch!=null)navigator.geolocation.clearWatch(state.gpsWatch);state.gpsWatch=null;state.gpsEnabled=false;state.autoArmed=false;$("gpsToggleBtn").textContent="📍 Activer le GPS";$("gpsToggleBtn").classList.remove("primary")}
